
import { GoogleGenAI, Type, Modality } from "@google/genai";
import { DesignAsset, Trend, MarketingCopy, MockupStyle } from "../types.ts";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

// Helper to handle API errors globally
const handleApiError = (error: any, context: string) => {
    console.error(`API Error in ${context}:`, error);
    
    if (error.status === 429 || error.message?.includes('429') || error.message?.includes('Quota') || error.message?.includes('RESOURCE_EXHAUSTED')) {
        throw new Error(`Gemini API Quota Exceeded. Please try again later or check your billing.`);
    }
    
    throw new Error(`AI Error: ${error.message || "Unknown error occurred"}`);
};

// Helper to convert image URL (http or data URL) to a generative part
const urlToGenerativePart = async (url: string, mimeType: string) => {
    const response = await fetch(url);
    const blob = await response.blob();
    const reader = new FileReader();
    const base64EncodedDataPromise = new Promise<string>((resolve) => {
        reader.onloadend = () => {
            const dataUrl = reader.result as string;
            // Handle cases where the data URL might not have a comma (though it should)
            const commaIndex = dataUrl.indexOf(',');
            if (commaIndex > -1) {
                resolve(dataUrl.substring(commaIndex + 1));
            } else {
                // If no comma, it might be just the base64 data already
                resolve(dataUrl);
            }
        };
        reader.readAsDataURL(blob);
    });
    const data = await base64EncodedDataPromise;
    return {
        inlineData: {
            data,
            mimeType,
        },
    };
};

/**
 * Generates a design image using AI.
 */
export const generateDesignImage = async (
  prompt: string,
  imagePart?: { inlineData: { data: string; mimeType: string } },
  textToAdd?: string
): Promise<string> => {
    
    try {
        let fullPrompt = `
            You are an expert digital artist creating isolated vector-style assets for Print-on-Demand products.
            Your task is to create a standalone graphic based on: "${prompt}".
        `;

        if (textToAdd) {
            if (textToAdd.toLowerCase().includes('quote') || textToAdd.toLowerCase().includes('message') || textToAdd.toLowerCase().includes('statement')) {
                 fullPrompt += ` The design should creatively incorporate a text phrase that fits the instruction: "${textToAdd}".`;
            } else {
                 fullPrompt += ` The design must include the text: "${textToAdd}". The text should be stylish, legible, and well-integrated.`;
            }
        }

        fullPrompt += `
            CRITICAL INSTRUCTIONS:
            1. Create ONLY the raw artwork/graphic.
            2. DO NOT draw a t-shirt, mug, poster, or any physical product.
            3. DO NOT show a model wearing the design.
            4. The background MUST be transparent (or solid white if transparency is not possible, but transparency is preferred).
            5. Focus on clean lines, high contrast, and commercial appeal.
        `;
        
        const response = await ai.models.generateImages({
            model: 'imagen-4.0-generate-001',
            prompt: fullPrompt,
            config: {
                numberOfImages: 1,
                outputMimeType: 'image/png',
                aspectRatio: '1:1'
            }
        });
        
        if (response.generatedImages && response.generatedImages.length > 0) {
            const base64Image = response.generatedImages[0].image.imageBytes;
            return `data:image/png;base64,${base64Image}`;
        } else {
            throw new Error("AI failed to generate an image.");
        }
    } catch (error) {
        handleApiError(error, "generateDesignImage");
        return ""; // Unreachable
    }
};

/**
 * Removes the background from an image.
 */
export const removeImageBackground = async (base64ImageUrl: string): Promise<string> => {
    try {
        const mimeType = base64ImageUrl.substring(base64ImageUrl.indexOf(":") + 1, base64ImageUrl.indexOf(";"));
        const imagePart = await urlToGenerativePart(base64ImageUrl, mimeType);
        const prompt = `
            Analyze the provided image and identify the main subject(s).
            Your task is to perfectly and cleanly remove the background from this image.
            CRITICAL: The final output MUST be only the subject(s) on a transparent background.
            Do not add any shadows, borders, or any other effects. Preserve all details of the subject.
        `;

        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [imagePart, { text: prompt }] },
            config: { responseModalities: [Modality.IMAGE] },
        });

        const imageOutputPart = response.candidates?.[0]?.content.parts.find(part => part.inlineData);
        if (imageOutputPart?.inlineData) {
            const { data, mimeType } = imageOutputPart.inlineData;
            return `data:${mimeType};base64,${data}`;
        }
        throw new Error("AI failed to remove the background.");
    } catch (error) {
        handleApiError(error, "removeImageBackground");
        return "";
    }
};

/**
 * Edits a design image based on a prompt.
 */
export const editDesignImage = async (base64ImageUrl: string, prompt: string): Promise<string> => {
    try {
        const mimeType = base64ImageUrl.substring(base64ImageUrl.indexOf(":") + 1, base64ImageUrl.indexOf(";"));
        const imagePart = await urlToGenerativePart(base64ImageUrl, mimeType);
        const fullPrompt = `
            Modify the provided image based on this instruction: "${prompt}".
            IMPORTANT: Preserve the transparent background. Do not add a new background.
            The output should be the modified image, ready for print.
        `;

        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [imagePart, { text: fullPrompt }] },
            config: { responseModalities: [Modality.IMAGE, Modality.TEXT] },
        });

        const imageOutputPart = response.candidates?.[0]?.content.parts.find(part => part.inlineData);
        if (imageOutputPart?.inlineData) {
            const { data, mimeType } = imageOutputPart.inlineData;
            return `data:${mimeType};base64,${data}`;
        }
        throw new Error("AI failed to edit the design.");
    } catch (error) {
        handleApiError(error, "editDesignImage");
        return "";
    }
};

/**
 * Generates an inverted (white) version of a design for dark products.
 */
export const generateInverseDesignVariant = async (base64ImageUrl: string): Promise<string> => {
    try {
        const mimeType = base64ImageUrl.substring(base64ImageUrl.indexOf(":") + 1, base64ImageUrl.indexOf(";"));
        const imagePart = await urlToGenerativePart(base64ImageUrl, mimeType);
        const prompt = `
            You are a print-on-demand design assistant. Your task is to invert a design for use on dark apparel.
            Analyze the provided image, which has a transparent background.
            Create a new version of this image where all black or dark-colored elements are converted to pure white (#FFFFFF).
            CRITICAL:
            1. The background MUST remain transparent. Do not add any background color.
            2. Preserve all original shapes, lines, and details. Do not change the art style.
            3. This is a color replacement task. Do not outline or redraw the image.
            4. The final output must be only the modified image on a transparent background.
            5. DO NOT draw a t-shirt. Only return the artwork.
        `;

        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [imagePart, { text: prompt }] },
            config: { responseModalities: [Modality.IMAGE, Modality.TEXT] },
        });

        const imageOutputPart = response.candidates?.[0]?.content.parts.find(part => part.inlineData);
        if (imageOutputPart?.inlineData) {
            const { data, mimeType } = imageOutputPart.inlineData;
            return `data:${mimeType};base64,${data}`;
        }
        throw new Error("AI failed to generate the design variant.");
    } catch (error) {
        handleApiError(error, "generateInverseDesignVariant");
        return "";
    }
};

/**
 * Generates a product mockup.
 */
export const generateMockupImage = async (
  designImageUrl: string,
  scene: string,
  color: string,
  productType: string,
  theme?: string
): Promise<string> => {
    try {
        const designPart = await urlToGenerativePart(designImageUrl, 'image/png');
        const prompt = `
            Create a single, high-quality, photorealistic mockup image.
            - Product Type: A ${productType}.
            - Product Color: The product must be the color ${color}.
            - The Scene: The scene must be: "${scene}".
            ${theme ? `- Thematic Elements: The scene should incorporate a subtle ${theme} theme.` : ''}

            CRITICAL INSTRUCTIONS:
            1. Place the following user-provided design onto the product.
            2. The design has a transparent background. You MUST treat the design as a final, non-editable decal.
            3. DO NOT change the colors of the design. DO NOT fill in any transparent areas of the design. Render it exactly as provided.
            4. The design must be realistically scaled, centered, and placed on the product. Avoid making it too large or distorted.
            5. The final image should be a professional, clean product photograph.
        `;

        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [designPart, { text: prompt }] },
            config: { responseModalities: [Modality.IMAGE, Modality.TEXT] },
        });

        const imagePart = response.candidates?.[0]?.content.parts.find(part => part.inlineData);
        if (imagePart?.inlineData) {
            const { data, mimeType } = imagePart.inlineData;
            return `data:${mimeType};base64,${data}`;
        }
        throw new Error("AI failed to generate a mockup image.");
    } catch (error) {
        handleApiError(error, "generateMockupImage");
        return "";
    }
};

/**
 * Generates a short video ad from a mockup image.
 */
export const generateVideoMockup = async (mockupImageUrl: string, style: MockupStyle): Promise<string> => {
    try {
        const imagePart = await urlToGenerativePart(mockupImageUrl, 'image/jpeg');

        const isPerson = ['man', 'woman', 'kid'].includes(style);
        const prompt = isPerson
            ? `
                Animate this photorealistic mockup image into a 5-second video ad.
                CRITICAL FRAMING: The camera frame MUST remain wide enough to clearly show the entire t-shirt design from the chest up at all times. DO NOT zoom in on the person's face. The product design is the hero.
                MOTION: The person in the image should make a subtle, natural movement like turning slightly, adjusting their pose, or a gentle, confident expression. The motion should be realistic and focus on the person enjoying wearing the product.
                STYLE: Keep it clean, professional, and focused on showcasing the apparel.
              `
            : `Create a clean, 5-second video ad from the provided mockup image. Use subtle, professional motion like a slow zoom, pan, or gentle 3D effect. CRITICAL: Do NOT add any text, logos, or extra graphics. The video should only feature the product mockup with natural-looking motion.`;

        let operation = await ai.models.generateVideos({
            model: 'veo-2.0-generate-001',
            prompt: prompt,
            image: {
                imageBytes: imagePart.inlineData.data,
                mimeType: imagePart.inlineData.mimeType,
            },
            config: { 
                numberOfVideos: 1,
                aspectRatio: '9:16', // Vertical video for social media ads (Stories/Reels)
            }
        });

        while (!operation.done) {
            await new Promise(resolve => setTimeout(resolve, 10000));
            operation = await ai.operations.getVideosOperation({ operation: operation });
        }

        const downloadLink = operation.response?.generatedVideos?.[0]?.video?.uri;
        if (!downloadLink) throw new Error("Video generation failed to return a download link.");

        const response = await fetch(`${downloadLink}&key=${process.env.API_KEY}`);
        if (!response.ok) throw new Error(`Failed to fetch video: ${response.statusText}`);
        
        const videoBlob = await response.blob();
        return URL.createObjectURL(videoBlob);
    } catch (error) {
        handleApiError(error, "generateVideoMockup");
        return "";
    }
};

export const generateTrendAnalysis = async (categoryOrTopic: string, platform: string = 'General'): Promise<Trend[]> => {
    try {
        const isGeneralDiscovery = !categoryOrTopic || categoryOrTopic.length < 3;
        
        let prompt = `
            You are a sophisticated e-commerce market analyst specializing in Print-on-Demand (POD).
            Your goal is to discover high-potential, actionable design niches for generic POD products (Graphics, Patterns, Typography).
            ${platform !== 'General' ? `Focus specifically on trends popular on ${platform}.` : ''}
        `;

        if (isGeneralDiscovery) {
            prompt += `
                TASK: Scan the current market landscape and identify 4 emerging, high-profit niches that are trending RIGHT NOW.
                Focus on specific sub-niches (e.g., "Retro Japanese Synthwave" instead of just "Music").
                Avoid specifying a product type (like "T-Shirt") in the niche name; focus on the DESIGN STYLE or TOPIC.
            `;
        } else {
            prompt += `
                TASK: Analyze the specific category/topic "${categoryOrTopic}".
                Identify 4 distinct, profitable sub-niches or design angles within this topic.
            `;
        }

        prompt += `
            For EACH niche, return a JSON object with:
            1. "niche": A catchy name for the trend.
            2. "description": Why is this trending? Who is buying it? (Target Audience).
            3. "opportunityScore": A number 1-100 based on demand vs competition.
            4. "keywords": 3-5 SEO tags.
            5. "imageExamples": 1 specific, detailed prompt for creating an isolated vector graphic in this style.
            
            Additional Data required for context:
            - "competitionLevel": "Low", "Medium", or "High".
            - "targetAudience": A specific persona (e.g., "Gen Z Gamers", "Cat Moms").
        `;

        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: prompt,
            config: {
                responseMimeType: 'application/json',
                responseSchema: {
                    type: Type.ARRAY,
                    items: {
                        type: Type.OBJECT,
                        properties: {
                            niche: { type: Type.STRING },
                            description: { type: Type.STRING },
                            opportunityScore: { type: Type.NUMBER },
                            competitionLevel: { type: Type.STRING },
                            targetAudience: { type: Type.STRING },
                            keywords: { type: Type.ARRAY, items: { type: Type.STRING } },
                            imageExamples: { type: Type.ARRAY, items: { type: Type.STRING, description: "A detailed prompt for generating this art style." } }
                        },
                        required: ["niche", "description", "opportunityScore", "keywords", "imageExamples", "competitionLevel", "targetAudience"],
                    }
                }
            }
        });
        const trendsData = JSON.parse(response.text) as any[];
        
        // NOTE: We removed the image generation loop here to save credits and speed up response.
        // We now return the text prompt in 'imageExamples' so the user can generate it manually later.
        
        return trendsData as Trend[];
    } catch (error) {
        handleApiError(error, "generateTrendAnalysis");
        return [];
    }
};

/**
 * Generates marketing copy for a given design asset.
 */
export const generateMarketingCopy = async (asset: DesignAsset): Promise<MarketingCopy> => {
    try {
        // We need to fetch the image to send it to the model for visual analysis
        // This ensures the copy matches the ACTUAL image content, not just the filename.
        const mimeType = asset.imageUrl.startsWith('data:image/png') ? 'image/png' : 'image/jpeg';
        const imagePart = await urlToGenerativePart(asset.imageUrl, mimeType);

        const prompt = `
            You are an expert e-commerce copywriter.
            Analyze the visual design in the attached image carefully.
            The design is named "${asset.name}" and has tags: ${asset.tags.join(', ')}.

            1. VISUAL ANALYSIS: First, look at the image. What is the main subject? What is the style?
            2. WRITING: Write specific, engaging marketing copy based on what you SEE in the design. Do not hallucinate subjects not present in the image.
            
            Create:
            - A catchy, SEO-friendly Title.
            - An engaging Product Description (2-3 sentences) that describes the visual design appealingly.
            - 5 relevant Tags/Keywords.
        `;
        
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: { parts: [imagePart, { text: prompt }] },
            config: {
                responseMimeType: 'application/json',
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        title: { type: Type.STRING, description: 'A catchy, short title for the product listing.' },
                        description: { type: Type.STRING, description: 'An engaging product description, 2-3 sentences long.' },
                        tags: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'A list of 5 relevant SEO tags or keywords.' }
                    },
                    required: ["title", "description", "tags"],
                }
            }
        });

        return JSON.parse(response.text) as MarketingCopy;
    } catch (error) {
        handleApiError(error, "generateMarketingCopy");
        return { title: '', description: '', tags: [] }; // Unreachable
    }
};
