    ...
    <div className={`bg-slate-800 rounded-lg overflow-hidden border-2 ${isSelected ? 'border-brand-primary' : 'border-slate-700'}`}>
        ...
        <div className="p-4">
            ...
            <div className="mt-2 text-sm text-slate-300">
                <div><span className="font-semibold text-slate-400">Base Price:</span> <span className="font-mono text-white">${(product.basePrice || 0).toFixed(2)}</span></div>
                {product.shipping && <div className="text-xs text-slate-500 mt-1">Shipping: {typeof product.shipping === 'string' ? product.shipping : JSON.stringify(product.shipping)}</div>}
                <div className="mt-2">
                    <div className="text-xs text-slate-400">Selling Price: <span className="font-mono text-white">${(product.sellingPrice ?? 0).toFixed(2)}</span></div>
                    <div className="text-xs text-slate-400">Total Cost: <span className="font-mono text-white">${(product.totalCost ?? product.basePrice ?? 0).toFixed(2)}</span></div>
                    <div className={`text-sm font-bold ${((product.sellingPrice ?? 0) - (product.totalCost ?? product.basePrice ?? 0)) > 0 ? 'text-green-400' : 'text-red-400'}`}>
                        Profit: ${( (product.sellingPrice ?? 0) - (product.totalCost ?? product.basePrice ?? 0) ).toFixed(2)}
                    </div>
                </div>
            </div>
